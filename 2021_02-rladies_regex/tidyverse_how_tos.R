...
names(metadatos) <- c("annio", "mes", "pais", "ciudad", "extensión")
...
conexiones
# Formamos los nombres de las columnas juntando las dos primeras filas
nombres <- paste0(conexiones[1,], "_", conexiones[2,])
names(conexiones) <- nombres
conexiones <- conexiones[-c(1,2), ]
conexiones

# Convertimos a formato ancho; ahora nosotrxs damos el 
# nombre de las columnas que queremos y como hay un separador
# fijo, lo aprovechamos (el guión bajo)
conexiones_longer <- conexiones %>% 
  fill(1) %>% 
  pivot_longer(cols = -c(1:2), names_to = c("annio", "mes"),
               values_to = "numero_de_conexiones", 
               names_sep = "_"
  )
conexiones_longer
